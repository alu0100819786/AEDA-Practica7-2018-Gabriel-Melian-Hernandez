...
public:
	...
	void insertar_balance(nodo<tipo_dato>* &nodo_pasado, nodo<tipo_dato>* nuevo, bool& crece);
	...
	void sustituir(nodo<tipo_dato>* &eliminado, nodo<tipo_dato>* &sustituto, bool &decrece);
...
template<class tipo_dato>
void arbol<tipo_dato>::insertar(tipo_dato elemento){
    nodo<tipo_dato>* nuevo = new nodo<tipo_dato>(elemento);
	bool crece = false;
	insertar_balance(raiz_, nuevo, crece);
}



template <class tipo_dato>
void arbol<tipo_dato>::insertar_balance(nodo<tipo_dato>* &nodo_pasado, nodo<tipo_dato>* nuevo, bool& crece){
	if (nodo_pasado == NULL){
		nodo_pasado = nuevo;
		crece = true;
	}
	else if (nuevo->dato() < nodo_pasado->dato()) {
		insertar_balance(nodo_pasado->izquierda(), nuevo, crece);
		if (crece) 
			insertar_rebalancea_izquierda(nodo_pasado, crece);
	}
	else {
		insertar_balance(nodo_pasado->derecha(), nuevo, crece);
		if (crece) 
...
template <class tipo_dato>
void arbol<tipo_dato>::insertar_rebalancea_izquierda(nodo<tipo_dato>* &nodo_pasado, bool &crece){
	switch (nodo_pasado->balance()){
		case -1:	
			nodo_pasado->balance() = 0;
			crece = false;
			break;

		case 0:		
			nodo_pasado->balance() = 1;
			break;

		case 1:		
			nodo<tipo_dato>* nodo_copia = nodo_pasado->izquierda();
			if (nodo_copia->balance() == 1)
...
template <class tipo_dato>
void arbol<tipo_dato>::insertar_rebalancea_derecha(nodo<tipo_dato>* &nodo_pasado, bool &crece){
	switch (nodo_pasado->balance()){
		case 1:		
			nodo_pasado->balance() = 0;
			crece = false;
...
template <class tipo_dato>
void arbol<tipo_dato>::eliminar_rama(nodo<tipo_dato>* &nodo_pasado, tipo_dato elemento, bool &decrece){
	if (nodo_pasado == NULL) 
		return;
	if (elemento < nodo_pasado->dato()) {
		...
		if (nodo_pasado->izquierda() == NULL) {
			...
			sustituir(eliminado, nodo_pasado->izquierda(), decrece);
			if (decrece)
				eliminar_rebalancea_izquierda(nodo_pasado, decrece);
		}
		delete eliminado;
...
template <class tipo_dato>
void arbol<tipo_dato>::eliminar_rebalancea_izquierda(nodo<tipo_dato>* &nodo_pasado, bool &decrece){
	switch (nodo_pasado->balance())	{
		case -1:{	
			nodo<tipo_dato>* nodo_copia = nodo_pasado->derecha();
			if (nodo_copia->balance() > 0)
				rotacion_DI(nodo_pasado);
			else {
				if (nodo_copia->balance() == 0)
					decrece = false;
...
template <class tipo_dato>
void arbol<tipo_dato>::eliminar_rebalancea_derecha(nodo<tipo_dato>* &nodo_pasado, bool &decrece){
	switch (nodo_pasado->balance())	{
		case 1:{	
			nodo<tipo_dato>* nodo_copia = nodo_pasado->izquierda();
			if (nodo_copia->balance() < 0)
...
template <class tipo_dato>
void arbol<tipo_dato>::sustituir(nodo<tipo_dato>* &eliminado, nodo<tipo_dato>* &sustituto, bool &decrece){
	if (sustituto->derecha() != NULL) {
		sustituir(eliminado, sustituto->derecha(), decrece);
		if (decrece)
			eliminar_rebalancea_derecha(sustituto, decrece);
	}
	else {
...
template<class tipo_dato>
bool arbol<tipo_dato>::balanceado(void){
	return balanceado(raiz_);
}

template<class tipo_dato>
bool arbol<tipo_dato>::balanceado(nodo<tipo_dato>* nodo){
	if(nodo == NULL)
		return true;
...
template<class tipo_dato>
void arbol<tipo_dato>::rotacion_II(nodo<tipo_dato>*& nodo_pasado){
	...
	if(nodo_copia->balance() == 1){
		nodo_pasado->balance() = 0;
		nodo_copia->balance() = 0;
	}
	else{
		nodo_pasado->balance() = 1;
		nodo_copia->balance() = -1;
	}
	nodo_pasado = nodo_copia;
}

template<class tipo_dato>
void arbol<tipo_dato>::rotacion_DD(nodo<tipo_dato>*& nodo_pasado){
	nodo<tipo_dato>* nodo_copia = nodo_pasado->derecha();
...
template<class tipo_dato>
void arbol<tipo_dato>::rotacion_ID(nodo<tipo_dato>*& nodo_pasado){
	nodo<tipo_dato>* nodo_copia = nodo_pasado->izquierda();
	...
	nodo_copia_2->balance() = 0;
	nodo_pasado = nodo_copia_2;
}

template<class tipo_dato>
void arbol<tipo_dato>::rotacion_DI(nodo<tipo_dato>*& nodo_pasado){
	nodo<tipo_dato>* nodo_copia = nodo_pasado->derecha();
...
void arbol<tipo_dato>::imprimir_dato(nodo<tipo_dato>* nodo){
	...
	cout << "[ " << nodo->dato() << " ]" << " ";
	cout << "Indice de Balanceo: " << nodo->balance() << " ";
}
